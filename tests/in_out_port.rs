...
macro_rules! test_getter_setter {
	($tp:ty, $name:expr, $value:expr, $value2:expr) => {
		...
		assert!(iop.set_src(iop.dest()).is_none());
		iop.set($value);
		let mut guard = iop.as_mut().unwrap();
...
fn getter_setter() {
	...
	test_getter_setter!(Vec<i32>, &p4_name, vec![1, 2, 3], vec![4, 5, 6]);
}
