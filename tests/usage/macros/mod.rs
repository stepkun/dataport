...
#[test]
fn macro_usage() {
	let _portlist = port_array!(inbound!("in", i32), inoutbound!("inout", i32), outbound!("out", i32));

	let _portlist = port_array!(
		inbound!("in", i32, 42),
		inoutbound!("inout", i32, 42),
		outbound!("out", i32, 42)
	);

	let _portlist = port_array!(inbound!("in", 42), inoutbound!("inout", 42), outbound!("out", 42));

	let _portlist = port_array!(inbound!("in", 2 * 42), inoutbound!("inout", 2 * 42), outbound!("out", 2 * 42));
